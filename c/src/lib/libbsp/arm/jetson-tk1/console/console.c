...
extern const car_dev tegra124_car_dev[ CAR_NUM_DEV ];
...
static ns8250_uart_context ns8250_uart_instances[] = {
  {
    .regs = UARTD,
    .car = &tegra124_car_dev[CAR_DEV_UARTD],
  ...
  {
    .regs = UARTA,
    .car = &tegra124_car_dev[CAR_DEV_UARTA],
...
static int ns8250_uart_poll_read( rtems_termios_device_context *context )
{
  ...
  /* Clear interrupts */
  mmio_write( ctx->regs + UART_LSR, 0 );
...
)
{
  ...
  mmio_write( ctx->car->reset.enb_clear,
              mmio_read ( ctx->car->reset.enb_clear ) | ctx->car->gate );
  mmio_write( ctx->car->src_reg, CAR_SOURCE_DEFAULT_UART);
  mmio_write( ctx->car->clock.enb_set, ctx->car->gate );
